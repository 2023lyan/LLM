...
def parse_args():
    ...
    parser.add_argument("--backward", action="store_true", help="Whether to benchmark backward pass.")
...
def benchmark(model, x, num_warmup, num_iters, backward):
    ...
    for _ in range(num_iters):
        ...
        times_forward.append(end_time_1 - start_time)
        if backward:
            loss = logits.sum()
            loss.backward()
            model.zero_grad()
            ...
            times_backward.append(end_time_2 - end_time_1)

    return times_forward, times_backward
...
if __name__ == "__main__":
    ...
    fwd_times, bwd_times = benchmark(transformer, data, args.num_warmup, args.num_iters, args.backward)
    print(f"The forward pass times record: {fwd_times}")
    print(f"The average forward pass time over {args.num_iters} iterations: {sum(fwd_times)/len(fwd_times):.4f} seconds")
    if args.backward:
        print(f"The backward pass times record: {bwd_times}")
        print(f"The average backward pass time over {args.num_iters} iterations: {sum(bwd_times)/len(bwd_times):.4f} seconds")
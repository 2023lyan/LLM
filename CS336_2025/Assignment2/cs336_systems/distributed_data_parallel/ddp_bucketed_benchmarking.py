...
def naive_ddp(rank, world_size, args, data, target, lr, loss_fn, bucket_size, results):
...
if __name__ == "__main__":
    ...
    for bucket_size in [1, 10, 100, 1000]:
        ...
        mp.spawn(
            naive_ddp,
            nprocs=world_size,
            args=(world_size, args, data, target, lr, loss_fn, bucket_size, results),
            join=True
        )
        total_time = results[0]
        print(f"bucket size: {bucket_size}MB, total_time: {total_time:.4f}s")